...
try:
    ...
    print(f"[ERROR] Missing required Python package '{e.name}'. Ask your developer to run: pip install -r requirements.txt")
    sys.exit(1)
...
def main():
    ...
    try:
        ...
        print(f"[ERROR] Could not authenticate with the service account file. {str(e)[:200]}")
        sys.exit(1)
    ...
    for i, url in enumerate(urls, start=1):
        ...
        if status != 200:
            ...
            reason = "the page could not be reached (network/timeout error)" if status is None else f"the page returned HTTP {status} instead of 200"
            print(f"[WARN] [{i}/{len(urls)}] Skipped: {url} — {reason} [HTTP {status}]")
        else:
            try:
                ...
                if content:
                    try:
                        detail = json.loads(content).get("error", {}).get("message", detail)
                    except (ValueError, AttributeError):
                        pass
                result = f"Error: {detail[:200]}"
                print(f"[ERROR] [{i}/{len(urls)}] {url} — {detail[:200]}")
