...
│   ├── sitemap_scraper.py       # used internally by the daily automation cron
│   ├── sitemap_url_extractor.py
│   ├── url_indexer.py
│   ├── indexing_checker.py
...
| # | Script | Input | Output |
|---|--------|-------|--------|
| 1 | **Sitemap URL Extractor** | Site name/URL | Downloadable CSV of every page URL (walks robots.txt → nested sitemap indexes) |
...
| 6 | **Sitemap Deleter** | GSC property + sitemap list | Deletion confirmation per sitemap |
| 7 | **GA4 Reporter** | Date range + property list | Traffic data logged to Google Sheet |

The **Lastmod Updater** (`/lastmod-updater`) is a separate, dedicated tool rather than a generic script — see below.

### Lastmod Updater

Pulls a domain's sitemap files from SFTP, S3, or a live site URL (via robots.txt discovery), lets you bump `<lastmod>`
on all files, a hand-picked selection, or an auto-detected "vertical" (a URL path pattern like `/product/{param}/`
grouping files that share it), and pushes the updated `sitemap-index.xml` to S3. Updates are file-level only — it
bumps the index's per-file `<lastmod>`, never rewrites the potentially 100M+ individual `<url>` entries inside each
child sitemap. SFTP/S3 credentials are configured on the Settings page. See `lib/lastmod/` for the implementation.
