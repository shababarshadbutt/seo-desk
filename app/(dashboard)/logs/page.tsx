...
export default async function LogsPage({
  ...
  if (q) {
    ...
    filter.$or = [{ scriptName: re }, { userName: re }, { websiteName: re }];
  }

  const [rawLogs, total, successCount, errorCount, durationAgg] = await Promise.all([
    ...
    ExecutionLog.aggregate([
      { $match: { ...filter, durationMs: { $ne: null } } },
      { $group: { _id: null, avg: { $avg: "$durationMs" } } },
    ]),
  ]);

  const avgDurationMs: number | null = durationAgg[0]?.avg ?? null;

  const logs: LogRow[] = rawLogs.map((l) => {
    const raw = l as unknown as Record<string, unknown>;
    const isAutomated = !!(raw.isAutomated);
    const script = getScriptBySlug(l.scriptSlug);
    const canRerun =
      !isAutomated &&
      !!script &&
      !script.requiresServiceAccount &&
      !script.inputs.some((i) => i.type === "file" || i.type === "multi-site-urls" || i.folder);
    return {
      id:          l._id.toString(),
  ...
  return (
    <Suspense>
      <LogsPageClient
        ...
        groups={groups}
        logs={logs}
        stats={{ total, success: successCount, error: errorCount, avgDurationMs }}
        from={from}
        to={to}
        page={page}
        total={total}
