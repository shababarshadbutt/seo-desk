...
import {
  ...
  Search, Clock, Download, RotateCw,
} from "lucide-react";
...
interface LogsPageClientProps {
  ...
  groups: GroupTab[];
  logs: LogRow[];
  stats: { total: number; success: number; error: number; avgDurationMs: number | null };
  from: string;
  to: string;
  page: number;
  total: number;
...
export function LogsPageClient({
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchInput !== q) navigate({ q: searchInput || undefined });
    }, 400);
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-end gap-2 flex-wrap">
          <Button variant="outline" size="sm" className="h-10" asChild>
            <a href={`/api/logs/export?${searchParams.toString()}`}>
      ...
      <div className="flex flex-wrap items-center gap-2 p-3 rounded-xl border bg-card">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search script or member…"
            className="h-8 w-52 rounded-lg border border-input bg-background pl-8 pr-2 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          />
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="rounded-xl border bg-card p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold leading-none text-emerald-600 dark:text-emerald-400">{stats.success}</p>
        ...
        <div className="rounded-xl border bg-card p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold leading-none text-rose-600 dark:text-rose-400">{stats.error}</p>
            <p className="text-xs text-muted-foreground mt-1">
        ...
        <div className="rounded-xl border bg-card p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold leading-none">
              {stats.avgDurationMs != null ? formatDuration(Math.round(stats.avgDurationMs)) : "—"}
            </p>
  ...
  return (
    <tr className="hover:bg-muted/30 transition-colors group">
      ...
      <td className="px-4 py-3 text-muted-foreground text-xs">
        {log.isAutomated ? (
          <div className="inline-flex items-center gap-1.5 rounded-full bg-violet-500/10 border border-violet-500/30 px-2.5 py-1 text-xs font-medium text-violet-700 dark:text-violet-400">
        ...
        <div className="flex items-center gap-1">
          <Dialog>
            ...
            <DialogContent className="max-w-3xl">
              <DialogHeader>
                ...
                <DialogDescription asChild>
                  <div className="flex items-center gap-3 pt-1 text-sm text-muted-foreground flex-wrap">
                    ...
                    {log.isAutomated ? (
                      <span className="inline-flex items-center gap-1 text-violet-700 dark:text-violet-400"><Zap className="h-3 w-3" />Automated{log.websiteName ? ` · ${log.websiteName}` : ""}</span>
                    ) : (
              ...
              <div className="rounded-xl bg-gray-950 p-4 font-mono text-xs text-gray-200 overflow-y-auto max-h-[420px]">
                {log.output ? (
                  log.output.split("\n").map((line, i) => (
          ...
          {log.canRerun && <RerunButton log={log} />}
        </div>
      </td>
    </tr>
  );
...
function RerunButton({ log }: { log: LogRow }) {
  const router = useRouter();
  ...
  async function runIt() {
    ...
    try {
      const res = await fetch(`/api/logs/${log.id}/rerun`, { method: "POST" });

      if (!res.ok || !res.body) {
        const data = await res.json().catch(() => null);
        setErrorMsg(data?.error ?? "Failed to start the re-run.");
        setBusy(false);
        return;
      }
      ...
      setErrorMsg("Connection lost while streaming the re-run.");
    } finally {
      setBusy(false);
      router.refresh();
    }
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!busy) setOpen(v); }}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={runIt}>
          <RotateCw className="h-3.5 w-3.5 mr-1" />Re-run
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>Re-run: {log.scriptName}</DialogTitle>
          <DialogDescription>
            Replaying the same inputs as the original run.
          </DialogDescription>
        </DialogHeader>
        <div className="rounded-xl bg-gray-950 p-4 font-mono text-xs text-gray-200 overflow-y-auto max-h-[420px]">
          {errorMsg ? (
            ...
            <span className="text-gray-500">{busy ? "Starting…" : "No output yet."}</span>
          ) : (
            lines.map((line, i) => (
              <div key={i} className={cn(
                "leading-5",
          ...
          {busy && <Loader2 className="h-3.5 w-3.5 animate-spin mt-2 text-gray-400" />}
        </div>
      </DialogContent>
    </Dialog>
  );
