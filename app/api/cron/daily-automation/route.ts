...
async function runAutomation() {
  ...
  // ── Prune old execution logs (retention configurable via Settings) ────────
  const retentionDays = settings?.logRetentionDays ?? 15;
  if (retentionDays > 0) {
    const cutoff = new Date(Date.now() - retentionDays * 24 * 60 * 60 * 1000);
    const { deletedCount } = await ExecutionLog.deleteMany({ startedAt: { $lt: cutoff } });
    if (deletedCount > 0) console.log(`[AUTOMATION] Pruned ${deletedCount} execution log(s) older than ${retentionDays} days`);
  } else {
    console.log("[AUTOMATION] Log retention set to unlimited — skipping prune.");
  }
